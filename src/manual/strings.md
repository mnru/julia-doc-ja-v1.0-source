...
```
  ...
  * 概念的には、文字列はインデックスから文字への **部分関数** です。
    ...
    こうすると、効率的な文字列のインデックス呼び出しが、エンコード表現に対するバイトインデックスによって可能になります。
    文字によるインデックスだと、ユニコード文字列はエンコーディングの変数の幅が変わるので、効率性と単純さを両立できないのです。
...
`Char`の値は1文字を表しています。
これは32bitのプリミティブ型で、特別なリテラル表現をもち、適当な算術が可能です。
...
変換した各値が有効な符号位置かどうか調べるには、[`isvalid`](@ref)関数を使ってください。
...
ユニコードとして入力可能なのは、一重引用符で囲んだ、`\u`のあとに４桁までの16進数を続けたものか、`\U`のあとに8桁までの16進数を続けたものです(最大の有効な桁数は6桁までですが)。
...
整数でインデックス付けされたオブジェクトの多くで、最初の要素のインデックスは１です。
(だからといって、あとで見るように、文字列の長さ`n`が、そのまま最後の要素のインデックスとなるわけでは、必ずしもありません。)
...
前者は型`Char`の文字１字の値、他方、後者はたまたま1文字しか含まない文字列の値です
Juliaではこれらは全く別物です。
