...
必ずしも必要ありませんが、変換した共通の型の値は元の値を全く忠実に表現していることが通常は想定されています。
この意味では、「昇格」という用語は適切で、値は「より大きな」型に変換されます。
つまり、入力した値のすべてを単一の共通型で表すことができます。
しかし重要な点ですが、昇格をオブジェクト指向の（構造的）スーパータイプやJuliaの抽象型のスーパータイプなどの概念と混同しないでください。
昇格は型の階層とは関連がなく、代替的な表現に変換するときのみに関わる概念です。
...
最も「賢明な」適用は、 `+`、 `-`、 `*` 、`/`のような算術演算子用の全捕捉メソッドの定義です。
...
昇格を行うのはこれらだけです。他の場所での算術演算の共通の数値型への昇格を心配する必要はありません。
自動的に処理されます。
...
-->
```
大抵のユーザー定義型では、プログラマーがコンストラクター関数に対して想定する型を明示的に指定するのがよいと思いますが、
時々、特に数値問題の場合は、自動昇格を使うと、便利なものです。
...
-->
```
原理的には`promote`関数のメソッドを直接定義することができますが、とりうるすべての引数の型の順列に対して多くの冗長な定義が必要になります。
